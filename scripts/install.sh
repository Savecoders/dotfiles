...
copy_and_cleanup() {
  ...
  mkdir -p "${dest_path}" || {
    echo "Error: Could not create destination directory ${dest_path}. Exiting."
    exit 1
  }
...
install_git_submodules() {
  ...
  # For Savior-zsh-theme
  clone_repo_temp https://github.com/Savecoders/Savior-zsh-theme.git
  copy_and_cleanup Savior-zsh-theme/* ../assets/zsh/Savior-zsh-theme/ true
...
install_paru() {
  ...
  (cd paru && makepkg -si) || {
    echo "Error: Failed to build and install paru. Exiting."
    exit 1
  }
  rm -rf paru
...
install_packages_from_list() {
  ...
  paru -S --noconfirm --needed - <"${package_list}" || {
    echo "Error: Failed to install packages from ${package_list}."
    return 1
  }
...
# Ask for Savior SDDM theme installation
ask_user "Do you want to install and configure Savior SDDM theme?" "install_sddm_theme"
...
mkdir -p "${HOME}/.config/" || {
  echo "Error: Could not create ~/.config/. Exiting."
  exit 1
}
...
echo "Copying Zsh assets..."
mkdir -p "${HOME}/.oh-my-zsh/themes/" || {
  echo "Error: Could not create ~/.oh-my-zsh/themes/. Exiting."
  exit 1
}
cp -r "${BASEDIR}/../assets/zsh/Savior-zsh-theme/"* "${HOME}/.oh-my-zsh/themes/" || {
  echo "Error: Failed to copy Savior-zsh-theme. Exiting."
  exit 1
}
cp "${BASEDIR}/../assets/zsh/.zshrc" "${HOME}/.zshrc" || {
  echo "Error: Failed to copy .zshrc. Exiting."
  exit 1
}
echo "Zsh resources copied."

# Fonts
echo "Copying fonts..."
mkdir -p "${HOME}/.local/share/fonts/" || {
  echo "Error: Could not create ~/.local/share/fonts/. Exiting."
  exit 1
}
cp -r "${BASEDIR}/../assets/fonts/"* "${HOME}/.local/share/fonts/" || {
  echo "Error: Failed to copy fonts. Exiting."
  exit 1
}
...
mkdir -p "${HOME}/Pictures/Wallpapers" || {
  echo "Error: Could not create ~/Pictures/Wallpapers. Exiting."
  exit 1
}
cp -r "${BASEDIR}/../assets/wallpapers/"* "${HOME}/Pictures/Wallpapers" || {
  echo "Error: Failed to copy wallpapers. Exiting."
  exit 1
}
...
mkdir -p "${HOME}/.icons" || {
  echo "Error: Could not create ~/.icons. Exiting."
  exit 1
}
...
extract_and_copy_icons() {
  local tar_file=$1
  local extract_dir="${BASEDIR}/../assets/icons/extracted_temp" # Temporary extraction dir
  ...
  if [[ "$tar_file" == *.zip ]]; then
    unzip -q "${BASEDIR}/../assets/icons/${tar_file}" -d "${extract_dir}" || {
      echo "Error: Failed to unzip ${tar_file}."
      return 1
    }
  elif [[ "$tar_file" == *.tar.xz || "$tar_file" == *.tar ]]; then
    tar -xvf "${BASEDIR}/../assets/icons/${tar_file}" -C "${extract_dir}" || {
